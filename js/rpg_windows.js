...
Window_Base.prototype.drawItemName = function(item, x, y, width) {
    ...
    if (item) {
        ...
        var itemMatch = /\C\[\d+\](.*?)\\C\[\d+\]/i.exec(item.name);
        this.resetTextColor();
        this.drawIcon(item.iconIndex, x + 2, y + 2);
        this.changeTextColor(this.textColor(this.obtainSkillEscapeParam(item.name)));
        if(itemMatch){
            this.drawText(itemMatch[1], x + iconBoxWidth, y, width - iconBoxWidth);
        }
        else
        {
...
Window_Base.prototype.obtainSkillEscapeParam = function(item) {
    if (item) {
        var match = /\\c\[(\d+)\]/i.exec(item);
