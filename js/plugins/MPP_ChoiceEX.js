...
Game_Interpreter.prototype.evalChoice = function(formula) {
    try {
        ...
        if(formula.slice(0,2) === "cm"){
            var formula2 = formula.replace(/cm\[(.*?)\]/g,
            (match, p1) => isNaN(parseInt(p1)) ? $CM_value.get(p1.toString()) : $CM_value.get(parseInt(p1)));
        }
        if(formula.slice(0,2) === "ST"){
            var formula2 = formula.replace(/ST\[(.*?)\]/g,
            (match, p1) => isNaN(parseInt(p1)) ? $ST_value.get(p1.toString()) : $ST_value.get(parseInt(p1)));
        }
